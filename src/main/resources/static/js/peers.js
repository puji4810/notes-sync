...
const PeersApp = {
    ...
    methods: {
        ...
        loadPeers() {
            ...
            axios.get('/api/v1/peers')
                ...
                .catch(error => {
                    ...
                    this.$notify.error({
                        title: '错误',
                        message: '获取节点列表失败: ' + (error.response?.data || error.message)
                    });
                })
                .finally(() => {
                    this.loading = false;
        ...
        // 添加新节点
        addPeer() {
            if (!this.newPeer.name || !this.newPeer.address) {
                this.$notify.warning({
                    title: '警告',
                    message: '请填写完整的节点信息'
                });
            ...
            axios.post('/api/v1/peers', this.newPeer)
                .then(() => {
                    this.$notify({
                        title: '成功',
                        message: '节点添加成功',
        ...
        connectToPeer(peer) {
            this.loading = true;
            axios.post(`/api/v1/peers/${peer.address}/connect`)
                .then(() => {
                    this.$notify({
                        title: '成功',
        ...
        disconnectFromPeer(peer) {
            this.loading = true;
            axios.post(`/api/v1/peers/${peer.address}/disconnect`)
                .then(() => {
                    this.$notify({
                        title: '成功',
                ...
                .catch(error => {
                    ...
                    this.$notify.error({
                        title: '错误',
                        message: '断开节点连接失败: ' + (error.response?.data || error.message)
                    });
                })
                .finally(() => {
                    this.loading = false;
        ...
        removePeer(peer) {
            ...
            axios.delete(`/api/v1/peers/${encodeURIComponent(peer.address)}`)
                .then(response => {
                    this.$notify({
                        title: '成功',
                        message: '节点已移除',
